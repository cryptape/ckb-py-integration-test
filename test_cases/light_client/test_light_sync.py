...
class TestLightSync(CkbTest):
    ...
    def test_sync(self):
        ...
        for i in range(10):
            ...
            new_sync_account = [self.Config.MINER_PRIVATE_1]
                ...
                setScripts.append(
                    {
                        ...
                        "script_type": "lock",
                        "block_number": hex(old_sync_block_number),
                    }
                )
            ...
            self.Node.wait_light_sync_height(
                self.ckb_light_node, add_sync_new_account_block_number, 1000000
            )
            # 3. sync new account
                ...
                newSetScripts.append(
                    {
                        ...
                        "script_type": "lock",
                        "block_number": hex(new_sync_block_number),
                    }
                )
            self.ckb_light_node.getClient().set_scripts(newSetScripts)
            print("until sync ")
            self.Node.wait_light_sync_height(
                self.ckb_light_node, new_sync_until_number, 100000
            )
            # 4. compare new account data
                ...
                light_cells = self.ckb_light_node.getClient().get_cells(
                    {
                        ...
                        "filter": {
                            "block_range": [
                                hex(new_sync_block_number),
                                hex(new_sync_until_number),
                            ]
                        },
                    },
                    "asc",
