...
class TestGetCells:
    ...
    def test_get_cells_output_data_filter_mode(self, get_cluster_indexer):
        ...
        output_data_prefix_results = get_cells_with_output_data(
            cluster, codehash, args, "0x01", "prefix"
        )
        assert output_data_prefix_results == ["0x0101", "0x0101"]

        output_data_prefix_results = get_cells_with_output_data(
            cluster, codehash, args, "0x02", "prefix"
        )
        assert output_data_prefix_results == ["0x0202", "0x0202"]

        # output_data_filter_mode: exact
        output_data_exact_results = get_cells_with_output_data(
            cluster, codehash, args, "0x0303", "exact"
        )
        assert output_data_exact_results == ["0x0303", "0x0303"]

        # output_data_filter_mode: partial
        output_data_partial_results = get_cells_with_output_data(
            cluster, codehash, args, "0x00000000ffff", "partial"
        )
        assert output_data_partial_results == [
            "0xffff00000000ffff",
            "0xffff00000000ffff",
        ]

        # script_search_mode: prefix
        search_mode_prefix_results = get_cells_with_script_search_mode(
            cluster, codehash, args, "prefix", "0x01", "prefix"
        )
        assert search_mode_prefix_results == ["0x0101", "0x0101"]

        # script_search_mode: exact
        search_mode_exact_results = get_cells_with_script_search_mode(
            cluster, codehash, args, "exact", "0x02", "prefix"
        )
        assert search_mode_exact_results == ["0x0202", "0x0202"]

        # script_search_mode: partial
        try:
            get_cells_with_output_data_byIndexer(
                cluster, codehash, args, "partial", "0x02", "prefix"
            )
        except Exception as e:
            print(f"Caught an exception: {e}")
            assert (
                "Invalid params the CKB indexer doesn't support search_key.script_search_mode partial search mode"
                in str(e)
            ), f"Unexpected exception: {e}"


def get_cells_with_output_data(
    cluster, codehash, args, output_data, output_data_filter_mode
):
    results = []
...
def get_cells_with_script_search_mode(
...
def get_cells_with_output_data_byIndexer(
    cluster, codehash, args, script_search_mode, output_data, output_data_filter_mode
):
    results = []
    ret = (
