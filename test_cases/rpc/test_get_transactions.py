...
class TestGetTransactions:
    ...
    def test_get_transactions(self, get_cluster_indexer):
        ...
        codehash = get_ckb_contract_codehash(deploy_hash, 0,
                                             ...
                                             api_url=cluster.ckb_nodes[0].getClient().url)
        # prefix search mode
        search_mode_prefix_results = get_transaction_with_script_search_mode(cluster, codehash, "0x02", "prefix")
        assert search_mode_prefix_results == ['0x454', '0x454']

        # exact search mode
        search_mode_exact_results = get_transaction_with_script_search_mode(cluster, codehash, "0x02", "exact")
        ...
        search_mode_partial_results = get_transaction_with_script_search_mode_byRichIndexer(cluster, codehash, "0x02",
                                                                                            "partial")
        ...
        for cell_data_filter_type in cell_data_filter_types:
            try:
                get_transactions_with_output_data_byIndexer(cluster, codehash, "0x02", "exact", "0x02",
                                                            cell_data_filter_type)
            except Exception as e:
                print(f"Caught an exception: {e}")
                assert "Invalid params doesn't support search_key.filter.output_data parameter" in str(e), f"Unexpected exception: {e}"


def get_transaction_with_script_search_mode_byRichIndexer(cluster, codehash, args, script_search_mode):
    results = []
    ret = cluster.ckb_nodes[1].getClient().get_transactions({
        "script": {
            "code_hash": codehash,
            "hash_type": "type",
            "args": args
...
def get_transaction_with_script_search_mode(cluster, codehash, args, script_search_mode):
    results = []
...
def get_transactions_with_output_data_byIndexer(cluster, codehash, args, script_search_mode,
                                               output_data, output_data_filter_mode):
    results = []
    ret = cluster.ckb_nodes[0].getClient().get_transactions({
        "script": {
            "code_hash": codehash,
            "hash_type": "type",
            "args": args
...
def get_transactions_with_output_data_byRichIndexer(cluster, codehash, args, script_search_mode,
                                                   output_data, output_data_filter_mode):
    results = []
    ret = cluster.ckb_nodes[1].getClient().get_transactions({
        "script": {
