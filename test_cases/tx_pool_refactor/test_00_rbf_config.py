...
class TestRBFConfig:
    def test_transaction_replacement_disabled_failure(self):
        ...
        :return:
        """
        node = CkbNode.init_dev_by_port(CkbNodeConfigPath.CURRENT_TEST, "tx_pool/node1", 8120,
                                        8225)
        node.prepare(other_ckb_config={"ckb_tx_pool_min_rbf_rate": "800"})
        node.start()
        make_tip_height_number(node, 30)
