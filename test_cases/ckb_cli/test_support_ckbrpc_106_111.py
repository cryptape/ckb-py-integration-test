...
class TestCkbCliSupport110(CkbTest):
    ...
    def test_01_version(self):
        """
        1. get ckb cli version
        Returns:

        """
        ckb_cli_version = self.Ckb_cli.version()
        assert (
            "ckb-cli" in ckb_cli_version
        ), "{ckb_cli_version} not contains ckb-cli".format(
    ...
    def test_02_estimate_cycles(self):
        ...
        result = self.Ckb_cli.estimate_cycles(
            "/tmp/tmp.json", api_url=self.node.getClient().url
        )
        assert result == 0
    ...
    def test_03_get_transaction_and_witness_proof(self):
        ...
        result = self.Ckb_cli.get_transaction_and_witness_proof(
            tx_hashes=cellbase_tx_hash,
            block_hash=None,
    ...
    def test_04_verify_transaction_and_witness_proof(self):
        ...
        block_number = self.node.getClient().get_tip_block_number()
        block = self.node.getClient().get_block_by_number(hex(block_number))
        tx_proof = self.node.getClient().get_transaction_and_witness_proof(
            tx_hashes=[block["transactions"][0]["hash"]]
        )
        with open("/tmp/tmp.json", "w") as tmp_file:
            print(f"tx_proof:{tx_proof}")
            tmp_file.write(json.dumps(tx_proof))
        result = self.Ckb_cli.verify_transaction_and_witness_proof(
            "/tmp/tmp.json", api_url=self.node.getClient().url
        )
    ...
    def test_05_get_block_with_cycles(self):
        ...
        block = self.node.getClient().get_block_by_number(hex(block_number))
        cellbase_block_hash = block["header"]["hash"]
        result = self.Ckb_cli.get_block(
            cellbase_block_hash, with_cycles=True, api_url=self.node.getClient().url
        )
        assert result == "[]"
    ...
    def test_06_get_block_by_number_with_cycles(self):
        ...
        """
        block_number = self.node.getClient().get_tip_block_number()
        result = self.Ckb_cli.get_block_by_number(
            block_number, with_cycles=True, api_url=self.node.getClient().url
        )
        assert result == "[]"
    ...
    def test_07_get_consensus(self):
        ...
        """
        hardfork_features = self.Ckb_cli.get_consensus(
            api_url=self.node.getClient().url
        )
        assert any(
            feature.get("rfc") == "0048"
            for sublist in hardfork_features
            for feature in sublist
        ), "不包含 RFC 协议 0048"
        assert any(
            feature.get("rfc") == "0049"
            for sublist in hardfork_features
            for feature in sublist
        ), "不包含 RFC 协议 0049"
