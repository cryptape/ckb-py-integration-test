...
class TestHopHint(FiberTest): #a-b
    ...
    def test_not_hophit(self):
        ...
        self.open_channel(self.fibers[2], self.fibers[3], 1000 * 100000000, 1) #c-d == b-c-d
        ...
        try:
            ...
            error_message = str(e)
            assert error_message == "Error: Send payment error: Failed to build route, PathFind error: no path found", \
    ...
    def test_use_hophit(self):
        ...
        payment = (
            self.fibers[1].get_client().get_payment({"payment_hash": payment_hash["payment_hash"]})
        )
        ...
        payment_hash = self.fibers[1].get_client().send_payment( #b
            {
                ...
                "amount": hex(10 * 100000000),
                "keysend": True,
                "hop_hints": [{"pubkey": self.fibers[3].get_client().node_info()["node_id"], #填的是 d 的 pubkey，表示在 d 节点使用 channel_outpoint 到 a
                              "channel_outpoint": da_channel_outpoint,
                              "fee_rate": 1000,
                              "tlc_expiry_delta": 1000}]
            }
