...
class TestRpc(CkbTest):
    ...
    def test_without_header(self):
        ...
        """
        ret113 = run_command(
            """curl -X POST  -d '[{"jsonrpc": "2.0", "method": "ping_peers", "params": [], "id": "1"}]' """
            + f"""{self.node113.rpcUrl}/ """
        )
        assert "null" in ret113
        ret = run_command(
            """curl -X POST  -d '[{"jsonrpc": "2.0", "method": "ping_peers", "params": [], "id": "1"}]' """
            + f"""{self.node112.rpcUrl}/ """
        )
    ...
    def test_max_batch_count(self):
        ...
        for rpcUrl in rpcUrls:
            ...
            for i in range(15376):
                requestBody = (
                    requestBody
                    + """{"jsonrpc": "2.0", "method": "ping_peers", "params": [], "id": "1"},"""
                )
            requestBody = requestBody[:-1]
            requests = (
                """curl -vvv -X POST -H "Content-Type: application/json" -d '["""
            ...
            assert (
                expected_error_message in exc_info.value.args[1]
            ), f"Expected substring '{expected_error_message}' not found in actual string '{exc_info.value.args[0]}'"
    ...
    def test_websocket(self):
        ...
        with pytest.raises(Exception) as exc_info:
            socket = self.node112.subscribe_websocket(
                "new_tip_header", self.node112.rpcUrl.replace("http://", "")
            )
