...
class TestHelperContract(CkbTest):
    ...
    def setup_class(cls):
        cls.node = cls.CkbNode.init_dev_by_port(
            cls.CkbNodeConfigPath.CURRENT_TEST, "helper/contract/node", 8114, 8115
        )
        cls.node.prepare()
        cls.node.start()
        ...
        cls.always_success_deploy_hash = cls.Contract.deploy_ckb_contract(
    ...
    def test_01_get_deploy_data_contract_hash(self):
        ...
        code_hash = self.Contract.get_ckb_contract_codehash(
            self.always_success_deploy_hash, 0, False, self.node.getClient().url
        )
        assert (
            code_hash
    ...
    def test_02_get_deploy_type_contract_hash(self):
        ...
        self.Contract.get_ckb_contract_codehash(
            self.always_success_deploy_hash, 0, True, self.node.getClient().url
        )
    ...
    def test_03_invoke_contract_use_type(self):
        ...
        assert (
            tx_response["transaction"]["outputs"][0]["type"]["code_hash"] == code_hash
        )
